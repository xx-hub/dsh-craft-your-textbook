...
import { generateContent, guessRoleFromName, resourceText } from './content-lib.js'
...
const PHASE_LABELS = {
  1: '材料准备', 2: '源探查', 3: '教学设计', 4: '最佳范例章', 5: '全章写作', 6: '终检与交付',
}
const GATE_LABELS = {
...
const STAGE_LABELS = {
  explore: '源探查', outline: '章节骨架', gold: '最佳范例章', chapters: '铺章', merge: '合并成书', final: '最后检查',
}
...
function workFile(projectId, name) {
  ...
  if (!target.startsWith(resolve(dir) + '\\') && target !== resolve(dir)) {
...
/** 宿主 ctx（apply 时赋值；appendEvent 播报主对话用）。 */
let hostCtx = null
  ...
  try {
    switch (action) {
      ...
      case 'pause': {
        ...
        try {
          hostCtx?.agents?.get?.(sessionId)?.cancel?.(
            { kind: 'hook', reason: meta.pause.reason },
            { keepInbox: true },
          )
        } catch (e) { hostCtx?.logger?.warn?.(`textbook: 暂停取消主 AI 失败: ${String(e instanceof Error ? e.message : e)}`) }
        // 子代理：中止当前回合（不 dispose、不重排队）；绝不 followup（中断即静默，恢复时 handoff 再带上下文）。
        // 注意：await 等子代理中断收敛完成后再回包（界面此时才显示「已暂停」，测试也依赖顺序）。
        try {
          const children = await hostCtx?.subagents?.listDescendants?.(sessionId) ?? []
          for (const child of children) {
            try { hostCtx.subagents.interrupt(child.childId, { kind: 'user', parentSessionId: sessionId }) }
            catch {
              try { hostCtx.subagents.interrupt(child.childId, { kind: 'ancestor', agent: hostCtx.agents.get(sessionId) }) }
              catch (e2) { hostCtx?.logger?.warn?.(`textbook: 中断子代理失败: ${String(e2 instanceof Error ? e2.message : e2)}`) }
            }
          }
        } catch (e) { hostCtx?.logger?.warn?.(`textbook: 枚举子代理失败: ${String(e instanceof Error ? e.message : e)}`) }
...
async function countDescendantSubagents(sessionId) {
  try {
    const svc = hostCtx !== null ? hostCtx.get('subagents') : undefined
...
async function handleProcess(req, res) {
  ...
  try {
    assertSessionOwned(project, sessionId)
    const subagents = await countDescendantSubagents(sessionId)
...
export function apply(ctx) {
  ...
  ctx.effect(() => ctx.webServer.register({ kind: 'exact', path: '/textbook/process', handler: handleProcess }), 'textbook: process route')
  ...
  // 宿主重启后：先回填老书的过程记录 → 处理重做标记 → 再自动续跑进行中的书。
  setTimeout(() => {
    backfillProcessLogs()
    processRedoMarks()
    resumeRunning(ctx)
