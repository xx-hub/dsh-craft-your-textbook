...
>
> - §七 金标准门槛 + §八 回填约束文件 = Iron Law 2 的卡关——金标准章通过四层审计 + 试教（条件触发，L1-L4 + L5）并回填 style-spec 前，禁止启动 Phase 5 并行。
...
> **真实模式落地（2026-08-27，ADR-0007）**：这一轮就是「合并前跨章审计」——主 AI 在机器合并成书**之前**通读全部章 + outline + knowledge-map，按下面的清单核对，结论落盘 `work/audit-cross.md`，机器验该文件存在才放行 `stage-submit merge`（`src/workflow.js` 硬校验）。机器轻量扫描（引用矛盾/跨章重复标题）与 AI 深度核对分工：机器扫结构、AI 查语义。
