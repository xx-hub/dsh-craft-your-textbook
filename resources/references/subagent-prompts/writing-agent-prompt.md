...
- **给全文**：`{{style-line.md 路径}}`（风格线镜像——用户对写法的要求汇总，通常是 `work/style-line.md`）。它与上面四份约束文件**同等地位**，不是"参考一下"。
- **动笔前逐条读**，写完对着每一条自查有没有落实；有没落实的在完工回报里写明是哪一条、为什么。
- **与 `style-spec.md` 打架时以风格线为准**（那是用户的原话），并在完工回报里点出冲突点让主 agent 裁决。
- **文件不存在 ≠ 这条不用管**：只说明用户还没提过风格意见；交章前让主 agent 用 `workbench_status` 查有没有新增的风格线（用户随时可能补）。
